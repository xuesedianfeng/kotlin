...
private fun KtNamedFunction.isAmbiguouslyDerived(): Boolean {
    val original = resolveToDescriptorIfAny()?.original
    ...
    if (overriddenDescriptors.any {
            it is JavaMethodDescriptor || when ((it.containingDeclaration as? ClassDescriptor)?.kind) {
                ClassKind.CLASS -> it.modality == Modality.ABSTRACT
                ClassKind.INTERFACE -> it.modality != Modality.ABSTRACT
                else -> false
            }
