...
abstract class KotlinGradleAbstractMultiplatformModuleBuilder : GradleModuleBuilder() {
    ...
    override fun setupModule(module: Module) {
        try {
            ...
            VfsUtil.saveText(buildGradle, builder.buildConfigurationPart() + builder.buildMainPart() + buildSourceSetPart())
        } finally {
            flushSettingsGradleCopy(module)
        }
    }

    protected abstract fun buildSourceSetPart(): String

    companion object {
        const val productionSuffix = "Main"

        const val presetsPrefix = "presets"
    }
}