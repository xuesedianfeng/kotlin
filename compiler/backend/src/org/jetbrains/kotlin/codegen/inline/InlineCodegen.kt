...
class PsiInlineCodegen(
    ...
    override fun genCallInner(
        ...
        val isSynchronized = resolvedCall?.resultingDescriptor?.isTopLevelInPackage("synchronized", "kotlin") == true
        if (isSynchronized) {
            state.globalCoroutinesContext.enterMonitor()
        }
        try {
            performInline(resolvedCall?.typeArguments, callDefault, codegen)
        } finally {
            if (isSynchronized) {
                state.globalCoroutinesContext.exitMonitor()
            }
