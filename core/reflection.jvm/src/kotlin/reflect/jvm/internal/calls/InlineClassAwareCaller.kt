...
internal class InlineClassAwareCaller<out M : Member>(
    ...
    private val data: BoxUnboxData by lazy(LazyThreadSafetyMode.PUBLICATION) {
        ...
        // maxOf is needed because in case of a top level extension, shift can be -1 (see above). But in that case, we need not unbox
        ...
        val unbox = arrayOfNulls<Method?>(expectedArgsSize)
        for (i in 0 until kotlinParameterTypes.size) {
            val inlineClass = kotlinParameterTypes[i].toInlineClass() ?: continue
            val index = i + shift
            if (index == -1) {
                // Do nothing. This means that we're looking at an extension receiver parameter in the case of a bound reference,
                // and in this case the bound receiver has been already unboxed at compilation time
            } else {
                if (index < 0) throw KotlinReflectionInternalError("Unexpected parameter index: $index (calling $descriptor)")
                unbox[index] = inlineClass.getUnboxMethod()
            }
        }
    ...
    override fun call(args: Array<*>): Any? {
        ...
        try {
            ...
            throw AssertionError("Range: $range\nUnbox: ${unbox.contentToString()}\nBox: $box\nArgs: ${args.contentToString()}")
        }
...
internal fun <M : Member> CallerImpl<M>.createInlineClassAwareCallerIfNeeded(
    ...
    if (descriptor.valueParameters.any { it.type.isInlineClassType() } ||
        descriptor.returnType?.isInlineClassType() == true
    ) {
        InlineClassAwareCaller(descriptor, this, isDefault)
    } else this
